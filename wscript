...
def build(bld):
    bld.stlib(source=bld.path.ant_glob("base/*.cpp"), target="base", includes=". base", use="PTHREAD")
    bld.program(source=bld.path.ant_glob("test/test*.cpp"), target="unittest", includes=".", use="base PTHREAD")
